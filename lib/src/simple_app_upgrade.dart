...
class _SimpleAppUpgradeWidget extends State<SimpleAppUpgradeWidget> {
  ...
  _clickOk() async {
    ...
    if (Platform.isIOS) {
      ...
      if (widget.iosAppId != null) HggUpgrade.toAppStore(widget.iosAppId!);
      return;
    }
    if (widget.downloadUrl == null || widget.downloadUrl!.isEmpty) {
      //没有下载地址，跳转到第三方渠道更新，原生实现
      HggUpgrade.toMarket(appMarketInfo: widget.appMarketInfo);
      return;
    }
    String path = await HggUpgrade.apkDownloadPath;
    ...
    try {
      ...
      await dio.download(url, path, onReceiveProgress: (int count, int total) {
        ...
        if (_downloadProgress == 1) {
          ...
          HggUpgrade.installAppForAndroid(path);
        }
      });
    } catch (e) {
      debugPrint('$e');
